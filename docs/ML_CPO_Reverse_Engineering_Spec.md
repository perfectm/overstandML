...
Train on an in-sample window of **IS months** (3 in the demo; a 2 appears in one config), predict the next **OoS days** (1), advance one day, repeat.

[CONFIRMED — Mauro direct, 26 Aug 2026] Mauro's own summary of the loop: (1) TRAIN on historical rows (features + target), then PREDICT on forward rows (features only). (2) IS = 3 months ≈ **62 trading days** live. (3) OoS = 1 day — one prediction per day. (4) A full walk-forward run (FWA) covers **all available data from May 2022 to present**: start at the beginning, TRAIN→PREDICT, shift the whole cycle forward 1 day (the OoS window), repeat. Example first cycle: IS = 16 May 2022 → 16 Aug 2022, OoS = 17 Aug 2022. The constant 3-month IS with a shifting start implies the live window is **rolling (unanchored)** — partially answering open question #5. **Anchored/Unanchored** toggle controls whether the train window start is fixed (expanding window) or rolls. The demo run: start date override 2025-10-01, first prediction 2026-01-01, data through 2026-08-18, producing **154 cycles** at initial equity $100,000. Modes: Monthly (Static) / Weekly (CPO) / **Daily (CPO)** — daily is the live mode.
